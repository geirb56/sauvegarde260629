...
- ✅ [2026-07-05] Quasi-real-time (Strava-like) feed via incremental batch sync — event-driven, no Kafka, Provider/frontend untouched. STRICT layering: `garmin_activities`=immutable ingestion SOURCE OF TRUTH; `workouts`=derived product layer built ASYNC via fan-out (sync worker NEVER writes workouts directly). Flow: gccli sync → write garmin_activities (dedupe external_id) → emit ACTIVITY_CREATED (Redis Stream `cardiocoach:events:activity_created`, capped 10k, group `workouts_fanout`) → `workers/event_worker.py` fan-out upserts workouts (keyed {id,user_id}) + updates Redis feed cache `cardiocoach:feed:{uid}`. New: `garmin/service.py::incremental_sync` (since=last_activity_ts), `GccliProvider.sync_activities` (smaller limit + since filter), `events/stream.py`, `feed/realtime_cache.py`, `sync/scheduler.py` (pure tiers ACTIVE 15m/NORMAL 2h/INACTIVE 24h), `sync/rate_limiter.py` (global cap 8 + per-user cooldown 900s), `workers/scheduler_worker.py` (leader-elected, enqueues only DUE users), API `GET /garmin/activities` (cache-first + `since`, backward compat) + `POST /garmin/activity-signal` (marks ACTIVE, no sync/gccli). Supervisor: added `event-worker`, `scheduler-worker`. Tests: `tests/test_sync_scheduler.py` ALL PASS; testing agent 14/14 PASS (iteration_24). Report: `memory/REALTIME_FEED_report.md`.
- ✅ [2026-07-05] SSE delivery layer `GET /api/garmin/feed/stream?user_id=` — READ-ONLY (no sync/gccli/DB writes). Streams ACTIVITY_CREATED filtered by user_id from the Redis Stream via non-destructive XREAD (fan-out group untouched; N clients/instances read independently → horizontally scalable). Reconnect-safe/idempotent via `Last-Event-ID` (or `?last_id=`); stateless handler; 1 dedicated Redis conn per stream (isolated from shared pool), closed on disconnect; `: ping` heartbeats. Files: `feed/sse.py`, endpoint in `api/garmin.py`. Tests `tests/test_sse.py` ALL PASS (delivery+filter, reconnect no-replay); verified live on localhost:8001. - ✅ [2026-07-05] Fixed multilingual LLM (chat coach + séance/bilan analyses were always English). Root causes: (1) backend `enrich_workout_analysis`/`enrich_weekly_review` never received a language → English-only prompts; (2) frontend `getAppLanguage()` defaulted to "en" with no browser detection. Fixes: `llm_coach.py` added `_lang_directive` + threaded `language` through enrich_workout_analysis/enrich_weekly_review/enrich_chat_response (strong "write ENTIRE response in <lang>" instruction on system+user prompts); `coach_service.py` analyze_workout/weekly_review accept+forward `language` (+language in cache key); `server.py` `/rag/workout/{id}` & `/rag/weekly-review` accept `language` param and pass it down; frontend `i18n.getAppLanguage` now auto-detects `navigator.language` (fr/es/en) when no stored preference, still overridable in Settings. Verified live: `/api/rag/weekly-review?language=fr` → French, `?language=en` → English.
