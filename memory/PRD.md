...
- ✅ [2026-07-01] Today's session now always coherent with Run Readiness (source of truth). Extracted pure `adapt_session_to_readiness()` + VMA pace helpers (`vma_pace`/`vma_pace_range`) in training_engine.py. RUN HARD=unchanged; EASY RUN=eases even endurance runs to 65-70% VMA (-15% duration, hard sessions→endurance); REST=active recovery 60-65% VMA (moderate, rr≥40) or full rest (high, rr<40). `adaptation_applied` true on any change. Added VMA confidence indicator (effort→high, average→medium, default→low) exposed in /training/plan and /training/today.
