...
agent_communication:
    ...
    - agent: "main"
      message: "NEW: Dashboard 'cardio-coach' block now computed from REAL Garmin data instead of _CARDIO_COACH_MOCK_DATA. New module backend/garmin/insights.py (compute_cardio_coach) wired into GET /api/cardio-coach (takes priority when garmin_connections.connected). Resting HR + sleep come from garmin_daily_metrics (real gccli data); training load / ACWR / fatigue_ratio computed from real garmin_activities (ACWR = 7d vs 28d daily load avg). HRV: used from real data when present (full fatigue formula), gracefully degraded (RHR+sleep+load reweighted, hrv fields null, reason 'HRV not recorded by your Garmin device') when the device doesn't record it. fatigue_ratio clamped >= 0. Please test: (1) GET /api/cardio-coach?user_id=default -> expect mock:false, source:'garmin', metrics.rhr_today is a real number (~44), metrics.sleep_hours real, metrics.training_load (ACWR) computed (>0), fatigue_ratio >= 0, hrv fields null + hrv_available:false, history is a 7-item array. (2) Confirm it is NOT the mock (mock must be false and 'source'=='garmin'). (3) Regression: GET /api/training/vma-history and /api/training/race-predictions (header X-User-Id: default) still return has_data:true (computed from real activities). (4) GET /api/workouts?user_id=default still returns real garmin workouts (data_source='garmin', no 'mock' ids). Note: real HRV path already verified manually (when hrv present, hrv_available:true and full HRV formula used). Provider is real gccli; token provisioned."
    - agent: "main"
