...
agent_communication:
    ...
    - agent: "main"
      message: "MOCK REMOVAL: deleted get_mock_workouts() entirely and removed its fallback from all 8 endpoints. Also previously deleted mock_runner. Please verify nothing is broken (default user has 30 real Garmin workouts in the workouts collection): (1) GET /api/workouts?user_id=default -> 200, returns ~30 workouts all data_source='garmin', NO mock. (2) GET /api/workouts?user_id=nonexistent_user_xyz -> 200, returns [] (empty list, NOT 500, NOT mock). (3) GET /api/workouts/{id} with a REAL garmin workout id (id starts with 'garmin-', get one from /api/workouts) -> 200 returns that workout; with a bogus id -> 404 (not mock). (4) GET /api/coach/digest?user_id=default&language=en -> 200 valid weekly review. (5) POST /api/coach/guidance {user_id:'default',language:'en'} -> 200 valid guidance. (6) GET /api/coach/workout-analysis/{garmin_id}?language=en -> 200; bogus id -> 404. (7) GET /api/coach/detailed-analysis/{garmin_id}?language=en -> 200; bogus id -> 404. (8) GET /api/cardio-coach?user_id=default -> still mock:false, source:'garmin'. (9) GET /api/dashboard?user_id=default -> 200. (10) Confirm /api/mock-runner -> 404 (removed). CRITICAL: no endpoint should 500 due to the removed mock; empty-data cases must degrade gracefully (empty list or 404). Read /app/test_result.md for context."
    - agent: "main"
